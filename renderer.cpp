...
// Translates mouse movements - rotation fixed
void Renderer::MouseProcessing(int button)
{
	...
	if (scn->isPicked )
	{
		...
		else
		{
			...
			scn->data().MyRotate(Eigen::Vector3d(1, 0, 0), yrel / 100.0);
			scn->data().MyRotate(Eigen::Vector3d(0, 1, 0), xrel / 100.0);
		}
	}
	else
	{
