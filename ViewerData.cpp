...
IGL_INLINE bool igl::opengl::ViewerData::my_collapse_edge(
    const int e,
    ...
    int& a_f2)
{
    ...
    Q4[E(e, 0)] = Q4[E(e, 0)] + Q4[E(e, 1)]; // add up the two matrices
    Q4[E(e, 1)] = Q4[E(e, 0)];
...
IGL_INLINE bool igl::opengl::ViewerData::my_collapse_edge(
    const std::function<void(
        ...
        double&,
        Eigen::RowVectorXd&,
        std::vector<Eigen::Matrix4d> Q4)>& cost_and_placement,
    Eigen::MatrixXd& V,
    Eigen::MatrixXi& F,
    Eigen::MatrixXi& E,
    Eigen::VectorXi& EMAP,
...
{
    int e1, e2, f1, f2;
    return my_collapse_edge(e, p, V, F, E, EMAP, EF, EI, e1, e2, f1, f2);
}
IGL_INLINE bool igl::opengl::ViewerData::my_collapse_edge(
    const std::function<void(
        const int,
    ...
    int& f2)
{
    ...
    {
        collapsed = my_collapse_edge(e, C.row(e), V, F, E, EMAP, EF, EI, e1, e2, f1, f2);
    ...
    if (collapsed)
    {
        printf("edge %d, cost = %f, new v position (%f, %f, %f)\n", e, p_cost, C.row(e)[0], C.row(e)[1], C.row(e)[2]);
        printf("edge %d, cost = %f, new v position (%f, %f, %f)\n", e1, Qit[e1]->first, C.row(e1)[0], C.row(e1)[1], C.row(e1)[2]);
        printf("edge %d, cost = %f, new v position (%f, %f, %f)\n\n", e2, Qit[e2]->first, C.row(e2)[0], C.row(e2)[1], C.row(e2)[2]);
        ...
        for (auto n : N)
        {
                ...
                F(n, 2) != IGL_COLLAPSE_EDGE_NULL)
            {
                for (int v = 0;v < 3;v++)
                {
                    ...
                    double cost;
                    RowVectorXd place;
                    cost_and_placement(ei, V, F, E, EMAP, EF, EI, cost, place, Q4);
                    // Replace in queue
