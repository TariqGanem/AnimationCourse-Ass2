...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	// number of collapsing
	int numOfIteration = std::ceil(0.05 * scn->getQ()[dataId]->size());
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case 'S':
			...
			break;
		case GLFW_KEY_UP:
			scn->data().v = Eigen::Vector3d(0, 0.01, 0);
			break;
		case GLFW_KEY_DOWN:
			scn->data().v = Eigen::Vector3d(0, -0.01, 0);
			break;
		case GLFW_KEY_LEFT:
			scn->data().v = Eigen::Vector3d(-0.01, 0,0);
			break;
		case GLFW_KEY_RIGHT:
			scn->data().v = Eigen::Vector3d(0.01, 0, 0);
			break;
		case '.':
			num = 0;
			// Simplification impl
			if (scn->getQ()[scn->data().id]->empty())
				break;
			dataId = scn->data().id;
			XV = scn->data().V;
			XF = scn->data().F;
			// reduce 5% of the edges
			for (int i = 0; i < std::ceil(0.05 * scn->getQ()[dataId]->size()); i++) {
				bool res = scn->data().my_collapse_edge(igl::quadric_error, XV, XF, scn->getE()[dataId],
					scn->getEMAP()[dataId], scn->getEF()[dataId], scn->getEI()[dataId], *(scn->getQ()[dataId]), scn->getQit()[dataId], *(scn->getC()[dataId]));
				if (!res)
					break;
				num++;
			}
			if (num > 0) printf("Collapsed %d edges\n", num);
			// updating the mesh to the new one
			scn->data().clear();
			scn->data().set_mesh(XV, XF);
			scn->data().set_face_based(true);
			scn->data().dirty = 157; //this line prevents texture coordinates update
			break;
		case ' ':
			if (scn->isMeshMoving) {
				scn->isMeshMoving = false;
				rndr->core().is_animating = false;
				scn->data_list[0].lastTree = scn->data_list[0].tree;
				scn->data_list[1].lastTree = scn->data_list[1].tree;
			}
			else {
				scn->isMeshMoving = true;
